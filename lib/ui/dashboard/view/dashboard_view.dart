...
// ⭐ UPDATED MENU ITEMS LIST: ADDING CATNA FORM CREATOR ⭐
...
    {'title': 'Employee List', 'icon': Icons.list, 'selected': false, 'path': Routes.dashboardEmployeeList},
    // New Menu Item
    {'title': 'Form Creator', 'icon': Icons.design_services_outlined, 'selected': false, 'path': Routes.dashboardCatnaCreator},
...
// NOTE: We now check if the full path contains the path segment defined in menuItems.
// We must use the 'get' function from Routes (e.g., Routes.getOverviewPath()) in the
// indexWhere check, because GoRouter's matched location includes the /dashboard prefix,
// but the item['path'] is just the segment (e.g., 'overview').
//
// However, since your current indexWhere relies on `location.contains(path)` where `path` 
// is the **segment**, and this worked before, we'll keep the logic the same to maintain 
// consistency, even though it's technically fragile. For the new item, the path segment is 
// 'form-creator'.
